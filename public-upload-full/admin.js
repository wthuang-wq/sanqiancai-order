...
const els = {
  ...
  batchProductSearch: document.querySelector("#batchProductSearch"),
  batchProductResults: document.querySelector("#batchProductResults"),
  ...
  addBatchItem: document.querySelector("#addBatchItem"),
...
async function loadProducts() {
  if (!els.productList && !els.priceEditor && !els.inventoryList && !els.batchProductSearch) return;
    ...
    .map(
      ([name, code]) => `
        <div class="inventory-row">
          ...
          <strong>${formatWeight(getStockQty(code))}</strong>
        </div>
      `,
    )
    .join("");
...
function renderBatchProductOptions() {
  if (!els.batchProductSearch || !els.batchProduct) return;
  if (els.batchProduct.value) return;
  els.batchProductSearch.value = "";
  renderPendingBatchItems();
}

function renderBatchProductResults(query = "") {
  if (!els.batchProductResults) return;
  const keyword = String(query || "").trim().toLowerCase();
  const matches = productList
    .filter(([name, code]) => !keyword || `${name} ${code}`.toLowerCase().includes(keyword))
    .slice(0, 12);

  if (!matches.length) {
    els.batchProductResults.hidden = false;
    els.batchProductResults.innerHTML = `<div class="product-search-empty">没有找到产品，请先去产品管理新增</div>`;
    return;
  }

  els.batchProductResults.hidden = false;
  els.batchProductResults.innerHTML = matches
    .map(
      ([name, code]) => `
        <button type="button" data-pick-batch-product="${escapeHtml(code)}" data-product-name="${escapeHtml(name)}">
...
function pickBatchProduct(code, name) {
  if (!els.batchProduct || !els.batchProductSearch || !els.batchProductResults) return;
  els.batchProduct.value = code;
  els.batchProductSearch.value = `${name} ${code}`;
  els.batchProductResults.hidden = true;
}

function getCurrentBatchItem() {
  const productNumber = els.batchProduct?.value || "";
  const selectedProduct = productList.find(([, code]) => code === productNumber);
  const quantity = Number(els.batchQuantity?.value || 0);
  if (!productNumber || !selectedProduct || !Number.isFinite(quantity) || quantity <= 0) return null;
  return {
    productNumber,
...
function clearBatchProductInputs() {
  if (els.batchProduct) els.batchProduct.value = "";
  if (els.batchProductSearch) els.batchProductSearch.value = "";
  if (els.batchQuantity) els.batchQuantity.value = "";
  if (els.batchProductResults) els.batchProductResults.hidden = true;
}

function renderPendingBatchItems() {
  if (!els.batchItems) return;
  if (!pendingBatchItems.length) {
    els.batchItems.innerHTML = `<div class="batch-item-empty">先添加本批号包含的产品</div>`;
    return;
  }

  els.batchItems.innerHTML = pendingBatchItems
    .map(
      (item, index) => `
        <article class="batch-item-row">
          <div>
            <strong>${escapeHtml(item.productName)} ${escapeHtml(item.productNumber)}</strong>
            <span>${formatWeight(item.quantity)}</span>
          </div>
          <button type="button" data-remove-batch-item="${index}">删除</button>
        </article>
      `,
    )
    .join("");
...
function addCurrentBatchItem() {
  const item = getCurrentBatchItem();
  if (!item) {
    showToast("请选择产品并填写入库数量");
    return false;
  }
  pendingBatchItems.push(item);
  clearBatchProductInputs();
  renderPendingBatchItems();
  return true;
}
      ...
      (batch) => `
        <article class="batch-row">
          <div>
            ...
            <span>批号：${escapeHtml(batch.batchCode)} · 入库日期：${escapeHtml(batch.producedAt || "未填")}</span>
...
function renderBatchSelector(order, item, index) {
  ...
  const allocatedBatches = Array.isArray(allocation?.batches)
    ? allocation.batches
  ...
  if (allocatedBatches.length) {
    return `
      <span class="batch-inline">
        批号：${allocatedBatches.map((batch) => `${escapeHtml(batch.batchCode)} ${formatWeight(batch.qty)}`).join("；")}
      </span>
    `;
  }

  if (currentRole !== "stock" || !["stock", "cooking"].includes(order.status)) return "";

  const productBatches = batches.filter((batch) => batch.productNumber === String(item.number || "").toUpperCase() && Number(batch.remaining || 0) > 0);
  const totalRemaining = productBatches.reduce((sum, batch) => sum + Number(batch.remaining || 0), 0);
  if (!productBatches.length) return `<span class="batch-inline warning">无可用批号，需先入库</span>`;
  return `<span class="batch-inline">发货时自动按旧批号先出 · 可用 ${formatWeight(totalRemaining)}</span>`;
}
...
els.batchProductSearch?.addEventListener("input", (event) => {
  els.batchProduct.value = "";
  renderBatchProductResults(event.target.value);
});

els.batchProductSearch?.addEventListener("focus", (event) => {
  renderBatchProductResults(event.target.value);
});

els.addBatchItem?.addEventListener("click", () => {
  addCurrentBatchItem();
});

els.batchItems?.addEventListener("click", (event) => {
  const removeButton = event.target.closest("[data-remove-batch-item]");
  if (!removeButton) return;
  const index = Number(removeButton.dataset.removeBatchItem);
...
document.addEventListener("click", (event) => {
  if (!els.batchProductResults || !els.batchProductSearch) return;
  const button = event.target.closest("[data-pick-batch-product]");
  if (button) {
    pickBatchProduct(button.dataset.pickBatchProduct, button.dataset.productName || "");
    return;
  }
...
els.batchForm?.addEventListener("submit", async (event) => {
  ...
  const currentItem = getCurrentBatchItem();
  if (currentItem) items.push(currentItem);

  if (!els.batchCode.value.trim()) {
    showToast("请先填写批号");
    return;
  }
  ...
  if (!items.length) {
    showToast("请至少添加一个产品");
    renderBatchProductResults(els.batchProductSearch?.value || "");
    return;
  }
...
async function init() {
  ...
  await loadBatches();
  if (els.orders || els.stockDoneOrders || els.shipmentNotices || els.newCount || els.salesCount || els.cookingCount || els.ownerCount || els.stockCount || els.totalCount) {
    await loadOrders();
