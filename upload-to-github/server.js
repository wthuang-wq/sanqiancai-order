...
async function deductBatchesForOrder(order) {
  const allocations = order.batchAllocations || {};
  const entries = Object.entries(allocations).filter(([, allocation]) => allocation?.batchId);
  if (!entries.length) return null;

  const batches = await readBatches();
  entries.forEach(([index, allocation]) => {
    const batch = batches.find((item) => item.id === allocation.batchId);
    if (!batch) return;
    const fallbackQty = getDeliveryItems(order)[Number(index)] || 0;
    const qty = Number(allocation.qty || fallbackQty || 0);
    batch.remaining = Math.max(0, Number(batch.remaining || 0) - qty);
...
async function handleApi(request, response, url) {
  ...
  if (request.method === "PATCH" && url.pathname === "/api/inventory") {
    ...
    Object.entries(items).forEach(([number, value]) => {
      const code = String(number || "").trim().toUpperCase();
  ...
  if (request.method === "POST" && url.pathname === "/api/batches") {
    if (!requireSession(request, response, ["stock", "owner", "admin"])) return true;
    const payload = await readJsonBody(request);
    const productNumber = String(payload.productNumber || "").trim().toUpperCase();
    const productName = String(payload.productName || "").trim();
    const batchCode = String(payload.batchCode || "").trim();
    const quantity = Number(payload.quantity);

    if (!productNumber || !batchCode || !Number.isFinite(quantity) || quantity <= 0) {
    ...
    const inventory = await readInventory();
    inventory[productNumber] = Number(inventory[productNumber] || 0) + quantity;
    await writeInventory(inventory);
  ...
  if (request.method === "PATCH" && statusMatch) {
    ...
    if (payload.batchAllocations && typeof payload.batchAllocations === "object") {
      order.batchAllocations = Object.fromEntries(
        Object.entries(payload.batchAllocations)
          .map(([index, allocation]) => [
            String(index),
            {
              batchId: String(allocation?.batchId || ""),
              batchCode: String(allocation?.batchCode || ""),
    ...
    if (status === "done" && previousStatus !== "done" && !order.inventoryDeductedAt) {
      ...
      await deductBatchesForOrder(order);
